...
// Function to get border fill color based on rating
function getBorderFillColor(ratingValue, label) {
  let value = parseFloat(ratingValue.replace('%', ''));

  // If rating is unavailable, the fill color for the '?' will be white
  if (ratingValue === '--') {
    return 'white';
  }

  if (label === 'IMDB') {
    if (value >= 8.0 && value <= 10) return '#28cd41'; // Light Green
    if (value >= 4.1 && value <= 7.9) return '#86c5e8'; // Cyan Blue
    if (value >= 0 && value <= 4.0) return 'red'; // Red
  } else if (label === 'Rotten Tomatoes') {
    if (value >= 80 && value <= 100) return '#28cd41'; // Light Green
    if (value >= 45 && value <= 79) return '#86c5e8'; // Cyan Blue
    if (value >= 0 && value <= 44) return 'red'; // Red
  }
...
function createRatingCircle(value, label, parentWidth, sizeMultiplier = 1) {
  ...
  if (value === '--') {
      ...
      const rtValue = value.replace('%', ''); // Get numerical part
      const percentSymbol = document.createElement('span');
      percentSymbol.textContent = '%';
      percentSymbol.className = 'percent-symbol'; // Apply class for smaller size
      text.textContent = rtValue; // Set the number part
      text.appendChild(percentSymbol); // Append the smaller '%'
...
const debouncedInjectRatings = debounce(injectRatings, 300);
